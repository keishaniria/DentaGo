...
<div class="container-fluid px-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h2 class="fw-bold text-dark">
            <i class="bi bi-clipboard-plus me-2"></i> Tambah Pemeriksaan
        </h2>
        <h5 class="text-muted">
            Pasien: <span class="fw-semibold">{{ $pasien->nama_pasien }}</span>
        </h5>
    </div>
...
<style>
    ...
    <div class="card shadow-sm border-0 pastel-card rounded-4">
        ...
        <div class="card-body p-4">
            <form action="{{ route('dokter.pemeriksaan.store', $pasien->id) }}" method="POST" enctype="multipart/form-data">
                @csrf

                <div class="mb-3">
                    <label class="form-label pastel-label fw-semibold">Keluhan</label>
                    <textarea class="form-control pastel-input" name="keluhan" rows="2" placeholder="Tuliskan keluhan pasien" required>{{ old('keluhan') }}</textarea>
                </div>

                <div class="mb-3">
                    <label class="form-label pastel-label fw-semibold">Diagnosa</label>
                    <textarea class="form-control pastel-input" name="diagnosa" rows="2" placeholder="Tuliskan hasil diagnosa dokter" required>{{ old('diagnosa') }}</textarea>
                </div>

                <div class="mb-3">
                    <label class="form-label pastel-label fw-semibold">Tindakan</label>
                    <textarea class="form-control pastel-input" name="tindakan" rows="2" placeholder="Tuliskan tindakan yang dilakukan" required>{{ old('tindakan') }}</textarea>
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="date" name="tanggal_pemeriksaan" class="form-control pastel-input" required value="{{ old('tanggal_pemeriksaan') }}">
                </div>
                ...
                <div class="d-flex justify-content-end">
                    <a href="{{ route('dokter.pasien.show', $pasien->id) }}" class="btn pastel-outline-secondary me-2 px-4">
